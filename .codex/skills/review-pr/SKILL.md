...
Poll every 60s using bash (first check at 8 min, timeout 15 min) until the bot's latest review `commit_id` matches HEAD. Use the same polling snippet from step 2. If timeout, tell the user to re-run the review-pr skill later.
