...
- On pay-per-token APIs, cheaper tiers cut real dollars. On subscriptions, each subagent rebuilds context whose findings flow back into yours, so total tokens can rise while the binding quota falls; the durable wins there are bucket arbitrage (e.g. Claude's separate Sonnet-only weekly allowance, while the frontier tier drains the shared bucket fastest), wall-clock parallelism, and a lean main context.
- Delegate in bounded waves (~3 parallel); between waves read the harness's own quota, never a third-party estimator (log-based tools such as ccusage guess spend and never see the real limit). Claude Code: the statusline JSON carries `rate_limits.five_hour.used_percentage`, `rate_limits.seven_day.used_percentage`, and `resets_at`; have the statusline script tee `.rate_limits` to `~/.claude/rate_limits.json` and read that file. Codex: the newest `~/.codex/sessions/**/*.jsonl` logs `rate_limits.primary.used_percent` (5h) and `secondary.used_percent` (weekly). Copilot: `gh api /copilot_internal/user --jq .quota_snapshots.premium_interactions`. Elsewhere treat the first rate-limit error as the cap. Stop launching once any usage window nears ~95%; let in-flight work finish.
- For long unattended runs, pause at the cap and resume when it clears: finish the wave, then use the harness's scheduled-wakeup primitive, chaining wakeups of ≤3600s until the window clears, never busy-waiting with `sleep`. If no such primitive exists, write a self-contained resume prompt to a handoff file (remaining plan, the 95% rule, the exact usage source and its last reading, subagent handoffs) and tell the user to relaunch with it. On resume, re-read the usage source: a `resets_at` later than the recorded one, not elapsed wall-clock, proves rollover. Tell the user which window tripped, the observed %, the next check time, and the outstanding work.
