...
Request re-review from the same bots found in step 2. Poll every 60s (first check at 8 min, timeout 15 min) until a new review's `commit_id` matches HEAD. If timeout, tell user to re-run `/review-pr` later.
